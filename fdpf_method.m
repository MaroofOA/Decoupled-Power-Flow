...
function [v, iteration] = fdpf_method(load_data, line_data, slack_bus_voltage, tolerance, max_iter, runs)
    ...
    for run = 1:runs
        ...
        for iteration = 1:max_iter % Iterate up to max_iter times to perform the BFS algorithm.
            ...
            % In Fast Decoubled Algorithm, J2 = J3 = 0, because in
            % transmission lines of power systems have a very low R/X
            % ratio. For such system, P_mismatch are less sensitive to
            % changes in the voltage magnitude and very sensitive to
            % changes in the phase angle. Similarly, Q_mismatch are less sensitive to
            % changes in the phase angle and very sensitive to
            % changes in the voltage magnitude.
            
            J1 = Jacobian_inv(1:num_buses-1, 1:num_buses-1);
            J2 = zeros(num_buses-1); % Making J2 zeros according to Fast decoupled 
            J3 = zeros(num_buses-1); % Making J3 zeros according to Fast decoupled
